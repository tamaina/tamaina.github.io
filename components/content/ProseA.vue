...
const linkTo = computed(() => {
  ...
  if (props.href.startsWith('/')) {
    return props.href.replaceAll(' ', '-').replaceAll('%20', '-');
  }

  const indexRegExp = /\/index\.(md|yml|yaml|json)$/;
  const isIndex = indexRegExp.test(page.value._file);

  return (isIndex ? `${page.value._path}/${props.href.startsWith('./') ? props.href.slice(2) : props.href}` : props.href).replaceAll(' ', '-').replaceAll('%20', '-');
});
</script>
